...
-- Functions
local function GetClosestRangerObject()
...
-- Events
RegisterNetEvent('ranger:client:spawnPObj', function(item)
    QRCore.Functions.Progressbar("spawn_object", Lang:t("progressbar.place_object"), 2500, false, true, {
        ...
        TriggerServerEvent("ranger:server:spawnObject", item)
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
...
RegisterNetEvent('ranger:client:deleteObject', function()
    local objectId, dist = GetClosestRangerObject()
    if dist < 5.0 then
        QRCore.Functions.Progressbar("remove_object", Lang:t('progressbar.remove_object'), 2500, false, true, {
            ...
            TriggerServerEvent("ranger:server:deleteObject", objectId)
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 1.0)
            QRCore.Functions.Notify(Lang:t("error.canceled"), 'error')
        end)
...
RegisterNetEvent('ranger:client:removeObject', function(objectId)
    NetworkRequestControlOfEntity(ObjectList[objectId].object)
    DeleteObject(ObjectList[objectId].object)
    ObjectList[objectId] = nil
end)

RegisterNetEvent('ranger:client:spawnObject', function(objectId, type, player)
...
RegisterNetEvent('ranger:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < Config.MaxSpikes then
        if PlayerJob.name == "ranger" and PlayerJob.onduty then
            ...
            TriggerServerEvent('ranger:server:SyncSpikes', SpawnedSpikes)
        end
    else
...
RegisterNetEvent('ranger:client:SyncSpikes', function(table)
    SpawnedSpikes = table
end)
