...
function TakeOutVehicle(vehicleInfo)
	...
	Citizen.InvokeNative(0x400F9556,veh, Lang:t('info.ranger_plate')..tostring(math.random(1000, 9999)))
...
function MenuGarage()
    ...
    for veh, label in pairs(authorizedVehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "",
            params = {
                event = "ranger:client:TakeOutVehicle",
                args = {
                    vehicle = veh
                }
            }
...
function CreatePrompts()
    for k,v in pairs(Config.Locations['duty']) do
        exports['qr-core']:createPrompt('duty_prompt_' .. k, v, 0xF3830D8E, 'Toggle duty status', {
            type = 'client',
            event = 'qr-rangerjob:ToggleDuty',
            args = {},
        })
    end
    
    for k, v in pairs(Config.Locations["vehicle"]) do
        exports['qr-core']:createPrompt("ranger:vehicle_"..k, vector3(v.x, v.y, v.z), Config.PromptKey, 'Jobgarage', {
            type = 'client',
            event = 'ranger:client:promptVehicle',
            args = {k},
        })
    end   
    ...
    for k,v in pairs(Config.Locations['evidence']) do
        exports['qr-core']:createPrompt('evidence_prompt_' .. k, v, 0xF3830D8E, 'Open Evidence Stash', {
            type = 'client',
            event = 'ranger:client:EvidenceStashDrawer',
            args = { k },
        })
    end
    ...
    for k,v in pairs(Config.Locations['stash']) do
        exports['qr-core']:createPrompt('stash_prompt_' .. k, v, 0xF3830D8E, 'Open Personal Stash', {
            type = 'client',
            event = 'ranger:client:OpenPersonalStash',
            args = {},
        })
    end
    ...
    for k,v in pairs(Config.Locations['armory']) do
        exports['qr-core']:createPrompt('armory_prompt_' .. k, v, 0xF3830D8E, 'Open Armory', {
            type = 'client',
            event = 'ranger:client:OpenArmory',
            args = {},
        })
    end
end
...
local function IsArmoryWhitelist() -- being removed
    ...
    if QRCore.Functions.GetPlayerData().job.name == 'ranger' then
        retval = true
    end
    return retval
end
...
RegisterNetEvent('ranger:client:promptVehicle', function(k)
    QRCore.Functions.GetPlayerData(function(PlayerData)
        PlayerJob = PlayerData.job
        onDuty = PlayerData.job.onduty
        local ped = PlayerPedId()

        if PlayerJob.name == "ranger"  then
...
RegisterNetEvent('ranger:client:ImpoundVehicle', function(fullImpound, price)
    ...
    if vehicle ~= 0 and vehicle then
        ...
        if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
            ...
            TriggerServerEvent("ranger:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage, totalFuel)
            QRCore.Functions.DeleteVehicle(vehicle)
        end
    end
end)

RegisterNetEvent('ranger:client:CheckStatus', function()
    QRCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "ranger" then
            local player, distance = GetClosestPlayer()
            if player ~= -1 and distance < 5.0 then
                local playerId = GetPlayerServerId(player)
                QRCore.Functions.TriggerCallback('ranger:GetPlayerStatus', function(result)
                    if result then
...
RegisterNetEvent('ranger:client:EvidenceStashDrawer', function(k)
...
RegisterNetEvent('qr-rangerjob:ToggleDuty', function()
    onDuty = not onDuty
    TriggerServerEvent("ranger:server:UpdateCurrentCops")
    TriggerServerEvent("ranger:server:UpdateBlips")
    TriggerServerEvent("QRCore:ToggleDuty")
end)

RegisterNetEvent('ranger:client:TakeOutVehicle', function(data)
    local vehicle = data.vehicle
    TakeOutVehicle(vehicle)
end)

RegisterNetEvent('ranger:client:OpenPersonalStash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "rangerstash_"..QRCore.Functions.GetPlayerData().citizenid)
    TriggerEvent("inventory:client:SetCurrentStash", "rangerstash_"..QRCore.Functions.GetPlayerData().citizenid)
end)

RegisterNetEvent('ranger:client:OpenPersonalTrash', function()
    TriggerServerEvent("inventory:server:OpenInventory", "stash", "rangertrash", {
        maxweight = 4000000,
        slots = 300,
    })
    TriggerEvent("inventory:client:SetCurrentStash", "rangertrash")
end)

RegisterNetEvent('ranger:client:OpenArmory', function()
    ...
    SetWeaponSeries()
    TriggerServerEvent("inventory:server:OpenInventory", "shop", "ranger", authorizedItems)
end)
...
CreateThread(function()
    if LocalPlayer.state.isLoggedIn and PlayerJob.name == 'ranger' then
        CreatePrompts()
    end
