...
AddEventHandler('QRCore:Client:OnPlayerLoaded', function()
    ...
    if PlayerJob and PlayerJob.name ~= "ranger" then
        if DutyBlips then
...
RegisterNetEvent('QRCore:Client:OnJobUpdate', function(JobInfo)
    if JobInfo.name == "ranger" and PlayerJob.name ~= "ranger" then
        CreatePrompts()
...
RegisterNetEvent('ranger:client:sendBillingMail', function(amount)
...
RegisterNetEvent('ranger:client:UpdateBlips', function(players)
    if Config.ShowBlips then
        if PlayerJob and (PlayerJob.name == 'ranger' or PlayerJob.name == 'ambulance') and onDuty then
            if DutyBlips then
                for k, v in pairs(DutyBlips) do
                    RemoveBlip(v)
                end
...
RegisterNetEvent('ranger:client:rangerAlert', function(coords, text)
    QRCore.Functions.Notify({text = text, caption = street1name.. ' ' ..street2name}, 'ranger')
    local transG = 250
...
RegisterNetEvent('ranger:client:SendToJail', function(time)
    TriggerServerEvent("ranger:server:SetHandcuffStatus", false)
    isHandcuffed = false
    isEscorted = false
    ClearPedTasks(PlayerPedId())
...
RegisterNetEvent('ranger:client:SendrangerEmergencyAlert', function()
    local Player = QRCore.Functions.GetPlayerData()
    TriggerServerEvent('ranger:server:rangerAlert', Lang:t('info.officer_down', {lastname = Player.charinfo.lastname, callsign = Player.metadata.callsign}))
