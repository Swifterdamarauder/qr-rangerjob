...
local function IsTargetDead(playerId)
    local retval = false
    QRCore.Functions.TriggerCallback('ranger:server:isPlayerDead', function(result)
        retval = result
    end, playerId)
    Wait(100)
    return retval
...
-- Events
RegisterNetEvent('ranger:client:SetOutVehicle', function()
...
RegisterNetEvent('ranger:client:PutInVehicle', function()
    local ped = PlayerPedId()
...
RegisterNetEvent('ranger:client:SearchPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("inventory:server:OpenInventory", "otherplayer", playerId)
        TriggerServerEvent("ranger:server:SearchPlayer", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)

RegisterNetEvent('ranger:client:SeizeCash', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("ranger:server:SeizeCash", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)

RegisterNetEvent('ranger:client:SeizeDriverLicense', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        TriggerServerEvent("ranger:server:SeizeDriverLicense", playerId)
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
end)
...
RegisterNetEvent('ranger:client:RobPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    local ped = PlayerPedId()
    if player ~= -1 and distance < 2.5 then
...
RegisterNetEvent('ranger:client:JailCommand', function(playerId, time)
    TriggerServerEvent("ranger:server:JailPlayer", playerId, tonumber(time))
end)

RegisterNetEvent('ranger:client:BillCommand', function(playerId, price)
    TriggerServerEvent("ranger:server:BillPlayer", playerId, tonumber(price))
end)

RegisterNetEvent('ranger:client:JailPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        local dialogInput = LocalInput(Lang:t('info.jail_time_input'), 11)
        if tonumber(dialogInput) > 0 then
            TriggerServerEvent("ranger:server:JailPlayer", playerId, tonumber(dialogInput))
        else
...
RegisterNetEvent('ranger:client:BillPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        local dialogInput = LocalInput(Lang:t('info.jail_time_input'), 11)
        if tonumber(dialogInput) > 0 then
            TriggerServerEvent("ranger:server:BillPlayer", playerId, tonumber(dialogInput))
        else
...
RegisterNetEvent('ranger:client:PutPlayerInVehicle', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("ranger:server:PutPlayerInVehicle", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('ranger:client:SetPlayerOutVehicle', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("ranger:server:SetPlayerOutVehicle", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('ranger:client:EscortPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not isHandcuffed and not isEscorted then
            TriggerServerEvent("ranger:server:EscortPlayer", playerId)
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
    end
...
RegisterNetEvent('ranger:client:KidnapPlayer', function()
    local player, distance = QRCore.Functions.GetClosestPlayer()
    if player ~= -1 and distance < 2.5 then
        local playerId = GetPlayerServerId(player)
        if not IsPedInAnyVehicle(GetPlayerPed(player)) then
            if not isHandcuffed and not isEscorted then
                TriggerServerEvent("ranger:server:KidnapPlayer", playerId)
            end
        end
    else
        QRCore.Functions.Notify(Lang:t("error.none_nearby"), 'error')
...
RegisterNetEvent('ranger:client:CuffPlayerSoft', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QRCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            local playerId = GetPlayerServerId(player)
            if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                TriggerServerEvent("ranger:server:CuffPlayer", playerId, true)
                -- HandCuffAnimation()
            else
...
RegisterNetEvent('ranger:client:CuffPlayer', function()
    if not IsPedRagdoll(PlayerPedId()) then
        local player, distance = QRCore.Functions.GetClosestPlayer()
        if player ~= -1 and distance < 1.5 then
            QRCore.Functions.TriggerCallback('QRCore:HasItem', function(result)
                if result then
                    local playerId = GetPlayerServerId(player)
                    if not IsPedInAnyVehicle(GetPlayerPed(player)) and not IsPedInAnyVehicle(PlayerPedId()) then
                        TriggerServerEvent("ranger:server:CuffPlayer", playerId, false)
                        -- HandCuffAnimation()
                    else
                        QRCore.Functions.Notify(Lang:t("error.vehicle_cuff"), 'error')
                    end
...
RegisterNetEvent('ranger:client:GetEscorted', function(playerId)
    local ped = PlayerPedId()
...
RegisterNetEvent('ranger:client:DeEscort', function()
    isEscorted = false
    TriggerEvent('hospital:client:isEscorted', isEscorted)
    DetachEntity(PlayerPedId(), true, false)
end)

RegisterNetEvent('ranger:client:GetKidnappedTarget', function(playerId)
    local ped = PlayerPedId()
    QRCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('ranger:client:GetKidnappedDragger', function(playerId)
    QRCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('ranger:client:GetCuffed', function(playerId, isSoftcuff)
    local ped = PlayerPedId()
    if not isHandcuffed then
        isHandcuffed = true
        TriggerServerEvent("ranger:server:SetHandcuffStatus", true)
        ClearPedTasksImmediately(ped)
        ...
        TriggerEvent('hospital:client:isEscorted', isEscorted)
        DetachEntity(ped, true, false)
        TriggerServerEvent("ranger:server:SetHandcuffStatus", false)
        ClearPedTasksImmediately(ped)
